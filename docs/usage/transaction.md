...
Can receive, as output information, the output of dashd's `listunspent` command, and a slightly fancier format recognized by unfycore:
...
Where `address` can be either a string or a unfycore Address object. The same is true for `script`, which can be a string or a unfycore Script.
